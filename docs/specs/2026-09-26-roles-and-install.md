...
| Integration worktree | `MAIN/.worktrees/integration` (the master also uses it for review branches) |
| Review branch (master only, short-lived) | `rip-swarm/review-<task_id>` |
...
| `task-available` | worker | `ID` holds no work claim and an open task exists that `ID` has not seen open before (§7.3). Detail: **one** task id, the first such task in id order; only that task is marked seen (§26 F1). |
| `task-finished` | master | A task gained a `complete`, `release`, `reject` or `expired` tombstone not seen before, **or** a task's claim has expired and not yet been stolen (seen once per expiry). A `complete` that was already on the board when this master joined, but is neither accepted nor rejected, counts as not seen (§7.5), so a new master reviews it once. Likewise a `reject` that an unsettled task still waits on directly, so a new master resumes an unfinished cascade. `wait` adds the tombstone (or the expiry) to the seen set **when it reports it**, as it does for `task-available`: each event is one wake, whatever the handler does with it. Detail: task id and action. |
...
- `wait` reports `task-available` for **one** `(task_id, generation)` pair not in the agent's seen set, the first in task-id order, and adds only that pair. A worker holds one claim (`max_claims_open_per_agent: 1`), so the other unseen open tasks stay unseen and are offered one per wake, at the following waits (§26 F1). A worker that passes on a task (D6) is not woken for it again; its next wait offers the next open task. A task that comes back after someone else's release or an expiry wakes every worker once more.
- **Own release.** When `release` is run by the agent that owns the local state, it adds `(task_id, G)` to that agent's seen set, where `G` is the task's generation computed **after** this release's tombstone exists. That agent is not re-offered a task it just gave up (for example `cannot merge integration`), while every other worker is. A later release or expiry by someone else produces a new generation and offers it again. A message that **asks this worker to claim** a task lets the worker claim it directly, without waiting for `task-available`. Mentioning a task id is not such a request: a question, a status note or a broadcast quoting an id changes nothing. The seen set is not changed by a message.
...
- **Seeded at join** (§4.1 step 9): `messages_cursor` is set to the newest message already present. `seen_tombstones` gets every `release`, `expired` and `reject` tombstone already present, and every `complete` tombstone whose task is already accepted or rejected. A bare `complete` (not accepted, not rejected) is **not** seeded, so the next master reviews it (§7.2). Nor is a `reject` tombstone that some task which is neither accepted nor rejected still names directly in its `after`: a master that died mid-cascade left that dependent blocked, so the next master is woken `task-finished <id> reject` and continues the cascade from there (§26 F3). `seen_open`, `seen_expiries` and `held` start **empty**. Old messages and completions are not replayed: a second goal on the same hive does not feed the first goal's completions into the new master's merge step. The current open board, including expired claims, **is** offered once, so a worker that joins after the plan was posted, a replacement worker, or one joining a takeover finds the work.
- Written atomically. If the file is later missing or unreadable, `wait` re-seeds it the same way (cursor and tombstones only, bare completes and still-awaited rejects excluded) and prints `NOTE state re-seeded` on stderr.
...
3. Loop:
   ...
   3. `task-available` → read the one offered task (`status`, `inbox/<id>.json`). Claim it if it fits the brief. Exit 2 means it is not yours: if the message says *retry*, re-run once; otherwise go back to 3.1. If it does not fit, go back to 3.1. Either way the next wake offers the next open task (§7.3).
   4. Exactly once per task, right after the claim succeeds (never after work for this task has started or been committed, §26 S1), in `WORKTREE` (which must be clean; leftovers of the worker's own earlier work are committed first), run `git merge --no-edit rip-swarm/integration` if that branch exists. Because `rip-swarm/<id>` is reused from task to task, if it has commits `rip-swarm/integration` lacks (an earlier result that was rejected, fell short, or is still unreviewed), the worker first keeps its tip reachable as `refs/rip-swarm/prev/<id>/<short sha>` and then runs `git reset --hard rip-swarm/integration` instead of the merge, so rejected work never rides into a later result (§25 I1).
...
5. Loop. Start `wait` in the background (§7.1) and end the turn. Heartbeat the baton before each merge, acceptance check and write, and again before half the lease has passed while a step is still running (§7.6). On each wake:
   - `task-finished complete` → if `T` is already accepted, do nothing: no merge, no review. If any task that `fixes` `T` is open, claimed, blocked or awaiting acceptance (the §7.4 list), skip: that follow-up decides `T`. Otherwise read the result ref (`rip-swarm/<id>@<sha>`) and review it **off** the integration branch, so no worker can merge unaccepted work:
     ...
     3. **Merged:** check the result against the task's acceptance check on the review branch.
        ...
        - **Falls short:** `git switch rip-swarm/integration`, `git branch -D rip-swarm/review-<T>`. Integration never contained the sha. It stays reachable on the worker's branch or, once that worker has started another task, under `refs/rip-swarm/prev/<worker>/<sha>`; either way the sha is all a follow-up needs. Post a follow-up with `--fixes T`, whose body names the sha to build on and the gap to close. Do not fix it yourself (D2).
...
- **wait:**
  ...
  - A blocked task is not open while any `after` task is merely complete; it opens only once every `after` task is **accepted**. `all-complete` is false while it is blocked.
...
- **Inbox `after`, accept, master reject:**
  - Unknown `after` and `fixes` ids are refused at creation. `claim` on a blocked task exits 2.
  - `claim` exits 2 with a distinct message for a task with a `complete` tombstone, a `reject` tombstone, or an acceptance record, and never creates an active claim beside a final tombstone.
...
## 17. Third review (2026-09-26) — revision 3

**Verdict:** needs revision (1 critical, 1 major, 2 minor).
**Reviewed against:** this document as revised (§1–§12, with §16's dispositions) and `rip_swarm/claim.py` / `rip_swarm/fold.py`. The acceptance race was reproduced with empty commits in a throwaway repo.
**§13 and §15 are not re-opened.** Seeding, master reject, the baton-undo order, `--path-format=absolute`, the heartbeat cadence and exit 3 all close the findings they name. The findings below are in the acceptance fold.

### Critical

#### C1. A shortfall reset does not remove the merge from any worker who merged integration during the review

§9 merges onto `rip-swarm/integration`, runs the acceptance check, and on a shortfall does `git reset --hard PRE`. The check is the long step §7.6 heartbeats through, so the unaccepted merge sits on the branch for the whole review. §8 step 4 has every worker who claims in that window run `git merge rip-swarm/integration`.

Reset moves the integration ref back to `PRE`. It does not rewrite the worker branch. That branch still has the unaccepted commits. When the master later merges the worker, those commits come back onto `rip-swarm/integration`. Accepting the worker's task then keeps them.

Reproduced with empty commits: integration reset to `PRE` (the bad commit was no longer on it); the worker branch still contained it; `git merge --no-ff` of the worker put `unaccepted work` back on integration.

Dependents stay blocked until `accept`, so they are not the path. Any other open task is. Parallel work is the normal plan (§9, the rehearsal). The rehearsal drives the helpers one at a time and can pass without anyone claiming during the check.

**Fix.** Do not merge onto `rip-swarm/integration` until the check has passed. Merge the result sha onto a review branch cut from the current integration tip (`rip-swarm/review-<task>`), and run the check there. Workers keep merging `rip-swarm/integration`, which has not moved.

- **Passes:** fast-forward `rip-swarm/integration` to the review tip (`--ff-only`, and only if it is still at the recorded tip), then `accept`, then delete the review branch.
- **Falls short:** delete the review branch. Integration never contained the sha. Post the follow-up as §9 already says.

`claim` and `wait` still do not need the project repo. This is the master's git step, same as the merge it already runs.

### Major

#### M1. A message that mentions a task id is a claim order, and `claim` will take a finished task

§7.3 and §8: a message that names a task is a request to claim it, and the worker may claim without waiting for `task-available`. That is wider than a retry. A question, a "task_X is done", or a `*` note that quotes an id all qualify, including for a worker who already passed on the task.

`try_claim` (`rip_swarm/claim.py`) checks that the inbox file exists and that the active claim is free or expired. It does not look at complete tombstones, reject tombstones, or (once it exists) `accepted/<T>.json`. The claim succeeds. `fold.has_final_tombstone` then treats an active claim beside a `complete` or `reject` tombstone as corrupt. `wait` will not offer that task, because it is not open. The message path bypasses `wait`, and §7.4 only adds a block check.

**Fix.** `claim` refuses, with exit 2 and a distinct message, when the task has a `complete` tombstone, a `reject` tombstone, or an acceptance record. A message lets the worker skip the seen set only when it asks for that task to be claimed. Mentioning the id is not itself that ask. Add both to the §11 claim tests.

### Minor

#### m1. One wait test still unblocks on `complete`

§11's wait bullet says a blocked task stays closed "until its `after` tasks complete". §7.4 and the accept tests say a `complete` tombstone keeps dependents blocked and only `accept` opens them. The wait bullet as written would lock M1 of §15 back in. Point it at acceptance.

#### m2. The follow-up worker never sees the sha the shortfall removed

After a shortfall, integration is back at `PRE` (and, with C1, it never left). §8 step 4 merges only `rip-swarm/integration`. The follow-up names the original sha, but the worker procedure never merges it, so a worker who follows the steps rebuilds from `PRE` and drops the unaccepted work. §9 should say the follow-up body tells the worker to merge that sha before editing.

---

## 18. Dispositions (revision 4, 2026-09-26)

Checked before folding: `try_claim` (`rip_swarm/claim.py`) checks only that the inbox file exists and whether the active claim is free, so a completed or rejected task can be claimed again. C1 is git semantics: resetting `rip-swarm/integration` does not rewrite a worker branch that already merged it.

| Finding | Disposition | Where |
|---------|-------------|-------|
| C1 | Accepted as proposed. The master merges and checks on `rip-swarm/review-<T>`, cut from the integration tip. Integration only fast-forwards to a passed review tip, so unaccepted work never reaches a branch that workers merge. | §1 Names, §9 |
| M1 | Accepted. `try_claim` refuses tasks with a `complete` or `reject` tombstone or an acceptance record (exit 2, distinct messages). Only a message that asks the worker to claim bypasses the seen set; a mention does not. | §7.3, §7.4, §8, §11 |
| m1 | Accepted. The wait test opens a blocked task only on acceptance. | §11 |
| m2 | Accepted. Follow-up and rebase task bodies name the sha to build on, and §8 step 4 merges it after integration. | §8 step 4, §9 |

---

## 19. Fourth review (2026-09-26) — revision 4

**Verdict:** needs revision (1 critical, 2 major, 1 minor).
**Reviewed against:** this document as revised (§1–§12, with §18's dispositions). `git branch -d` on an unmerged review branch was reproduced in a throwaway repo.
**§13, §15 and §17 are not re-opened.** The review branch keeps unaccepted work off `rip-swarm/integration`, and `claim` refusing a finished task closes M1 of §17. The findings below are what that fold left unfinished.

### Critical

#### C1. A later master never reviews work that is complete but not yet accepted

§7.5 seeds `seen_tombstones` with every tombstone already present. §7.2 fires `task-finished` only for a tombstone that is not in that set. A complete tombstone with no `accepted/<T>.json` and no reject tombstone is therefore invisible to any master who joins after it exists.

That is the handoff §9 describes. `lease-lost` stops the current master and leaves the plan on the board for the next one. The next `/swarm-master` seeds the complete tombstone, does not wake, and `all-complete` stays false because the task is not accepted or rejected (§7.2). `idle-board` does not fire: the task is not open. `timeout` only starts `wait` again. The goal sits until someone nudges.

The same hole drops the follow-up link. `accept --via` is something the master remembers from the turn that posted `F`. Nothing on the board says `F` is what makes `T` acceptable. A new master who did wake for `T` would review it again while a worker is also doing `F`.

**Fix.** Seed a complete tombstone only when that task is already accepted or rejected. Release, expired and reject tombstones are still seeded. A bare complete is `task-finished complete` for the next master, once. Give the follow-up and the rebase task a create-only inbox field `fixes: <T>` (same rule as `after`: the id must exist). On accepting a task that `fixes` `T`, also `accept --task T --via <that task>`. While such a task is still open or blocked, do not review `T` again.

### Major

#### M1. `git branch -d` refuses the review branch on every shortfall and every conflict

§9 says "delete the review branch" after a conflict, a shortfall, or (by omission) a reject. Those branches are not contained in `rip-swarm/integration`. Git's ordinary delete then exits 1:

```text
error: the branch 'rip-swarm/review-T' is not fully merged
```

Reproduced. The pass path fast-forwards first, so `-d` works there. The shortfall path, which the rehearsal requires to drop the branch, does not. A literal `git branch -d` is a non-zero exit in the middle of the review. The reject bullet never switches back to `rip-swarm/integration` either, so a later synthesis commit can land on the review branch, on top of the work that was just rejected.

After a real delete, the sha is still on `rip-swarm/<worker>`. That is what the follow-up merges. It is not on integration.

**Fix.** Switch the integration worktree back to `rip-swarm/integration` before any delete, including the reject bullet. Delete with `git branch -D`. A review branch left by a crash is removed the same way before `git switch -c`, unless it is already the merge of this sha onto the recorded tip, in which case the check continues on it.

#### M2. The rebase task conflicts by construction, and step 4 releases on that conflict

§9's conflict follow-up tells the worker to merge the sha that just conflicted with integration. §8 step 4 merges integration, merges that sha, and on a conflict in either merge aborts and releases. The worker never reaches step 5, so the conflict is not the work. It is another release.

Own-release (§7.3) then hides that generation from the worker who just bounced. The next worker claims it, merges the same pair, and releases too. When every live worker has done that, nobody is offered the task. The master treats `task-finished release` as nothing required. `all-complete` stays false. `idle-board` is the only remaining signal, and it asks the operator to nudge.

**Fix.** A task whose body says to merge a named sha, or whose `fixes` field is set, is allowed to conflict on that merge. The worker resolves the conflict in the worktree and completes. Abort-and-release stays the rule for an unexpected conflict while merging `rip-swarm/integration` at the start of an ordinary task. The rehearsal should complete the rebase task, not only post it.

### Minor

#### m1. The rehearsal says the unaccepted sha ends on no branch

§11: after the review branch is dropped, "the sha is on neither branch." It remains on `rip-swarm/<worker>` for as long as that branch does (M1). Say it is on the worker branch and not on `rip-swarm/integration` or any review branch.

---

## 20. Dispositions (revision 5, 2026-09-26)

| Finding | Disposition | Where |
|---------|-------------|-------|
| C1 | Accepted as proposed. A bare `complete` (not accepted, not rejected) is not seeded, so the next master reviews it once. A create-only `fixes: <T>` field puts the follow-up link on the board: accepting `F` accepts `T --via F` along the chain, and `T` is not re-reviewed while a `fixes` task is live. | §7.2, §7.4, §7.5, §9 |
