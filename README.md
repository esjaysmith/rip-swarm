...
Cross-harness agent swarm coordination via a **git-backed hive** (not a hosted queue). Each project keeps its hive on a dedicated **`swarm` branch** of its own repo, mounted as a git worktree at **`_swarm/`** (gitignored on code branches) and pushed to the project’s normal remote.
